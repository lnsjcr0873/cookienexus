...
class SessionManager:
    ...
    def inject_httpx(self, httpx_client: Any, domain: str) -> None:
        """
        Injects cookies directly into an httpx.Client or httpx.AsyncClient.
        """
        cookies = self.client.get_cookies(domain)
        for c in cookies:
            httpx_client.cookies.set(c['name'], c['value'], domain=c.get('domain'), path=c.get('path', '/'))

    def inject_requests(self, requests_session: Any, domain: str) -> None:
        """
